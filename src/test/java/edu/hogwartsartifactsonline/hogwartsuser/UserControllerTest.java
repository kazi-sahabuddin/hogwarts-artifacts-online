...
class UserControllerTest {
    ...
    void testFindUserByIdErrorWithNonExistentId() throws Exception {
        //Given
        given(this.userService.findById(1)).willThrow( new ObjectNotFoundException("user",5));
    ...
    void testFindUserByIdSuccess() throws Exception {
        ...
        h.setEnabled(true);
        h.setRoles("user");
        given(this.userService.findById(1)).willReturn(h);
        ...
        this.mockMvc.perform(get(this.baseUrl+"/users/1").accept(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("$.data.id").value(h.getId()))
                .andExpect(jsonPath("$.data.username").value(h.getUsername()));
    }
    ...
    void testAddUserSuccess() throws Exception {
        ...
        given(this.userService.save(Mockito.any(HogwartsUser.class))).willReturn(newUser);
        ...
        this.mockMvc.perform(post(this.baseUrl+"/users").contentType(MediaType.APPLICATION_JSON).content(json).accept(MediaType.APPLICATION_JSON))
                ...
                .andExpect(jsonPath("$.data.id").value(newUser.getId()))
