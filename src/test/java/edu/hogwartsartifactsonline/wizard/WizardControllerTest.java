...
class WizardControllerTest {
    ...
    void testUpdateArtifactSuccess() throws Exception {
        //Given
        WizardDto wizardDto = new WizardDto(eq(1), "Albus Dumbledore", null);
        String json = this.objectMapper.writeValueAsString(wizardDto);
    ...
    void testUpdateWizardErrorWithNonExistentId() throws Exception {
        //Given
        WizardDto wizardDto = new WizardDto(eq(9), "Albus Dumbledore", null);
        String json = this.objectMapper.writeValueAsString(wizardDto);

        given(this.wizardService.update(9, Mockito.any(Wizard.class))).willThrow(new ObjectNotFoundException("wizard", 9));
        //When and Then
    ...
    void testDeleteWizardErrorWithNonExistentId() throws Exception {
        //Given
        doThrow(new ObjectNotFoundException("wizard",10)).when(this.wizardService).delete(10);

        //When and Then
        this.mockMvc.perform(delete(this.baseUrl+"/wizards/10").accept(MediaType.APPLICATION_JSON))
                .andExpect(jsonPath("$.flag").value(false))
                .andExpect(jsonPath("$.code").value(StatusCode.NOT_FOUND))
                .andExpect(jsonPath("$.message").value("Could not find wizard with Id 10 :("))
                .andExpect(jsonPath("$.data").isEmpty());
    }
}